...
function updatePlayer(dt, p, input) {
  if(p == null)
    return;
	if(input.keys['a'] || input.joystick.dx < 0)
		p.vx = -p.speed;
	else if(input.keys['d'] || input.joystick.dx > 0)
		p.vx = p.speed;
  else
		p.vx = 0;
  if(input.keys['w'] || input.joystick.dy > 0)
		p.vy = -p.speed;
	else if(input.keys['s'] || input.joystick.dy < 0)
		p.vy = p.speed;
  else
		p.vy = 0;
	p.x += p.vx * p.vsx;
