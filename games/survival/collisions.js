...
const COLLISION_VALID_TARGETS = new Set(["enemy", "car", "trashcan", "animal", "rocket"]);
const COLLISION_POTENTIAL_TARGETS = new Set(["player", "enemy", "car", "trashcan", "animal", "rocket"]);
const PLAYER_SHIELDS = [
    { key: 'shield_blue_health', kE: 1.0, kO: 0.95 },
    { key: 'shield_green_health', kE: 0.25, kO: 0.75 },
    { key: 'shield_shadow_health', kE: 0.25, kO: 0.75 },
    { key: 'shield_rainbow_health', kE: 0.10, kO: 0.55 },
    { key: 'shield_anubis_health', kE: 0.10, kO: 0.55 }
];
...
function collisions_handle_pair_unified(g, objA, objB, dt) {
    ...
    if (sName === "bullet" || sName === "trashbullet" || oName === "bullet" || oName === "trashbullet") {
        ...
        if (bData.enemy) {
            if (tName === "player") {
                collisions_apply_damage_to_player(target.data, bData.damage, dt, "bullet", g);
                bData.damaged = true;
            }
            ...
            collisions_apply_damage_to_object(g, target, bData.damage, dt, true);
            bData.damaged = true;
    ...
    if (sName === "rocket" || oName === "rocket") {
        ...
        if (COLLISION_POTENTIAL_TARGETS.has(tName)) {
            ...
            if (tName === "player" && rData.enemy) {
                collisions_apply_damage_to_player(tData, rData.damage, dt, "rocket", g);
            } else {
                collisions_apply_damage_to_object(g, target, rData.damage, dt, !rData.enemy);
            }
    ...
    if (sName === "enemy" || oName === "enemy") {
        ...
        if (tName === "player") {
            collisions_apply_damage_to_player(tData, eData.damage, dt, "enemy", g);
        } else if (tName === "car") {
            ...
            if ((!eData.boss && speedSq > 0.81 * maxSpeed * maxSpeed) || (tData.is_tank && speedSq > 0.01 * maxSpeed * maxSpeed)) {
    ...
    if ((sName === "car" && oName === "animal") || (sName === "animal" && oName === "car")) {
...
function collisions_apply_damage_to_player(p, damage, dt, source, g) {
    ...
    if (source === "rocket") {
        ...
        k = 0.05;
        if (typeof hotbar_get_selected_item === 'function' && hotbar_get_selected_item(p.hotbar_element) === ITEM_HORN) {
            k = 0.001;
        }
    }
    ...
    if (p.health <= 0) {
        ...
        if (source === "enemy") {
            DEATH_MESSAGE = isRu ? "☠️ игрок был заражён" : "☠️ the player has been infected";
        } else if (source === "bullet") {
            DEATH_MESSAGE = isRu ? "☠️ игрок был подстрелен" : "☠️ the player was killed by a bullet";
        } else if (source === "rocket") {
            DEATH_MESSAGE = isRu ? "☠️ игрок был подстрелен самонаводящейся ракетой" : "☠️ the player was killed by a rocket";
        }
    }
}
...
function collisions_apply_damage_to_object(g, obj, damage, dt, isFromPlayer) {
    ...
    if (isFromPlayer && (obj.name === "enemy" || obj.name === "rocket")) {
        ...
        history.push({ dmg: finalDmg, time: now });
