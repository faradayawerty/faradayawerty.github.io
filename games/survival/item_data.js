...
let ITEM_RENDERERS = {
	...
	[ITEM_GREEN_GUN]: (ctx, x, y, w, h, animstate) => {
		...
		const drawLeech = (startX, startY, maxAmp, freq, phase,
			...
			for (let i = 0; i <= segments; i++) {
				...
				const wave = Math.sin(t_anim * freq + t * Math.PI *
					phase) * maxAmp * damping;
		...
		ctx.fillRect(x + w * 0.42, y + h * 0.55 + slosh, w * 0.11, h *
			0.15 - slosh);
		ctx.fillStyle = darkGreen;
		ctx.fillRect(x + w * 0.88, y + h * 0.4, w * 0.06, h * 0.16);
		ctx.lineCap = "butt";
	},
	[ITEM_GUN]: (ctx, x, y, w, h) => {
		ctx.fillStyle = "#333";
	...
	[ITEM_SWORD]: (ctx, x, y, w, h) => {
		...
		for (let i = 1; i < 4; i++) {
			...
			drawLine(ctx, -hW / 2, ry, hW / 2, ry, "rgba(0,0,0,0.3)",
				1);
		}
		ctx.restore();
	},
	[ITEM_RED_PISTOLS]: (ctx, x, y, w, h) => {
	...
	[ITEM_RAINBOW_PISTOLS]: (ctx, x, y, w, h, animstate) => {
		let drawSinglePistol = (px, py, scale, color) => {
			ctx.fillStyle = color;
			ctx.fillRect(px, py, w * 0.35 * scale, h * 0.12 *
				scale);
	...
	[ITEM_PLASMA_LAUNCHER]: (ctx, x, y, w, h) => {
		ctx.fillStyle = "#331133";
	...
	[ITEM_ROCKET_LAUNCHER]: (ctx, x, y, w, h) => {
		ctx.fillStyle = "#111133";
		ctx.fillRect(x + w * 0.05, y + h * 0.3, w * 0.9, h * 0.45);
		ctx.fillStyle = "#000";
		ctx.fillRect(x + w * 0.3, y + h * 0.3, w * 0.4, h * 0.1);
	...
	[ITEM_OLD_SHOE]: (ctx, x, y, w, h) => {
		ctx.fillStyle = "#553311";
		ctx.fillRect(x + 0.2 * w, y + 0.6 * h, 0.6 * w, 0.2 * h);
		ctx.fillRect(x + 0.2 * w, y + 0.3 * h, 0.3 * w, 0.4 * h);
	},
	[ITEM_BENT_FORK]: (ctx, x, y, w, h) => {
		ctx.strokeStyle = "#aaa";
		ctx.lineWidth = 2;
		drawLine(ctx, x + 0.5 * w, y + 0.8 * h, x + 0.5 * w, y + 0.4 *
			h, "#aaa");
		drawLine(ctx, x + 0.4 * w, y + 0.2 * h, x + 0.4 * w, y + 0.4 *
			h, "#aaa");
		drawLine(ctx, x + 0.6 * w, y + 0.1 * h, x + 0.6 * w, y + 0.4 *
			h, "#aaa");
	},
	[ITEM_JUNK_CANNON]: (ctx, x, y, w, h) => {
		ctx.fillStyle = "#444";
		...
		ctx.lineWidth = 3;
		drawLine(ctx, x + 0.4 * w, y + 0.3 * h, x + 0.4 * w, y + 0.65 *
			h, "blue");
	},
	[ITEM_CRUMPLED_PAPER]: (ctx, x, y, w, h) => {
		...
		ctx.fill();
		ctx.stroke();
		drawLine(ctx, x + 0.4 * w, y + 0.4 * h, x + 0.6 * w, y + 0.5 *
			h, "#dddddd", 1);
	},
	[ITEM_DEAD_BATTERY]: (ctx, x, y, w, h) => {
	...
	[ITEM_APPLE]: (ctx, x, y, w, h) => {
		ctx.fillStyle = "#ff4422";
		ctx.beginPath();
		ctx.ellipse(x + 0.5 * w, y + 0.55 * h, 0.24 * w, 0.24 * h, 0, 0,
			Math.PI * 2);
		ctx.fill();
		ctx.strokeStyle = "#440000";
		ctx.lineWidth = 0.03 * w;
		ctx.stroke();
		drawLine(ctx, x + 0.5 * w, y + 0.35 * h, x + 0.5 * w, y + 0.15 *
			h, "#442200", 0.04 * w);
		ctx.fillStyle = "#33aa11";
		ctx.beginPath();
		ctx.ellipse(x + 0.6 * w, y + 0.22 * h, 0.1 * w, 0.05 * w, -Math
			.PI / 4, 0, Math.PI * 2);
		ctx.fill();
		ctx.fillStyle = "rgba(255, 255, 255, 0.6)";
		ctx.beginPath();
		ctx.ellipse(x + 0.4 * w, y + 0.45 * h, 0.07 * w, 0.07 * w, 0, 0,
			Math.PI * 2);
		ctx.fill();
	},
	[ITEM_APPLE_CORE]: (ctx, x, y, w, h) => {
		ctx.fillStyle = "#eeeecc";
		ctx.beginPath();
		ctx.ellipse(x + 0.5 * w, y + 0.5 * h, 0.08 * w, 0.2 * h, 0, 0,
			Math.PI * 2);
		ctx.fill();
		ctx.fillStyle = "#ff4422";
		ctx.fillRect(x + 0.4 * w, y + 0.3 * h, 0.2 * w, 0.05 * h);
		ctx.fillRect(x + 0.4 * w, y + 0.65 * h, 0.2 * w, 0.05 * h);
		drawLine(ctx, x + 0.5 * w, y + 0.3 * h, x + 0.5 * w, y + 0.15 *
			h, "#442200", 0.03 * w);
	},
	[ITEM_FISH_BONE]: (ctx, x, y, w, h) => {
		ctx.strokeStyle = "#dddddd";
		ctx.lineWidth = 0.05 * w;
		drawLine(ctx, x + 0.2 * w, y + 0.5 * h, x + 0.8 * w, y + 0.5 *
			h, "#dddddd");
		for (let i = 0; i < 4; i++) {
			drawLine(ctx, x + (0.3 + i * 0.15) * w, y + 0.35 * h, x + (
				0.3 + i * 0.15) * w, y + 0.65 * h, "#dddddd");
		}
	...
	[ITEM_TIN_CAN]: (ctx, x, y, w, h) => {
		...
		ctx.lineWidth = 1;
		drawLine(ctx, x + 0.3 * w, y + 0.45 * h, x + 0.7 * w, y + 0.45 *
			h, "#777777");
		drawLine(ctx, x + 0.3 * w, y + 0.55 * h, x + 0.7 * w, y + 0.55 *
			h, "#777777");
		drawLine(ctx, x + 0.3 * w, y + 0.65 * h, x + 0.7 * w, y + 0.65 *
			h, "#777777");
		ctx.fillStyle = "#999999";
		ctx.beginPath();
		ctx.ellipse(cx, y + 0.75 * h, 0.2 * w, 0.05 * h, 0, 0, Math.PI *
			2);
		ctx.fill();
		ctx.stroke();
		ctx.fillStyle = "#444444";
		ctx.beginPath();
		ctx.ellipse(cx, y + 0.35 * h, 0.2 * w, 0.05 * h, 0, 0, Math.PI *
			2);
		ctx.fill();
	...
	[ITEM_CHERRIES]: (ctx, x, y, w, h) => {
		...
		drawCircle(ctx, x + 0.65 * w, y + 0.7 * h, 0.12 * w, "#ee0000",
			"#660000", 0.02 * w);
		drawCircle(ctx, x + 0.32 * w, y + 0.65 * h, 0.03 * w, "white",
			"none", 0);
		drawCircle(ctx, x + 0.62 * w, y + 0.65 * h, 0.03 * w, "white",
			"none", 0);
	},
	[ITEM_CHICKEN_LEG]: (ctx, x, y, w, h) => {
		drawLine(ctx, x + 0.5 * w, y + 0.6 * h, x + 0.5 * w, y + 0.85 *
			h, "#eeeeee", 0.08 * w);
		...
		ctx.beginPath();
		ctx.ellipse(x + 0.5 * w, y + 0.4 * h, 0.2 * w, 0.28 * h, 0, 0,
			Math.PI * 2);
		ctx.fill();
		ctx.fillStyle = "#aa6622";
		ctx.beginPath();
		ctx.ellipse(x + 0.45 * w, y + 0.35 * h, 0.12 * w, 0.18 * h, 0,
			0, Math.PI * 2);
		ctx.fill();
	},
	...
	[ITEM_AMMO]: (ctx, x, y, w, h) => {
		...
		for (let i = 0; i < N; i++) {
			...
			ctx.strokeRect(x + i * w / N + 0.25 * w / N, y + 0.25 * h,
				0.5 * w / N, 0.5 * h);
		}
	},
	[ITEM_SHIELD_RAINBOW]: (ctx, x, y, w, h, animstate) => {
		if (animstate == null) return;
		let r = Math.floor(Math.pow(Math.cos(0.1 * animstate) * 15, 2));
		let g = Math.floor(Math.pow(0.7 * (Math.cos(0.1 * animstate) +
			Math.sin(0.1 * animstate)) * 15, 2));
		let b = Math.floor(Math.pow(Math.sin(0.1 * animstate) * 15, 2));
	...
	[ITEM_RAINBOW_AMMO]: (ctx, x, y, w, h, animstate) => {
		if (animstate == null) return;
	...
	[ITEM_BOSSIFIER]: (ctx, x, y, w, h, animstate) => {
		if (animstate == null) return;
		let rate = 0.05;
		let r = Math.floor(Math.pow(Math.cos(rate * animstate) * 15,
			2));
		let g = Math.floor(Math.pow(0.7 * (Math.cos(rate * animstate) +
			Math.sin(rate * animstate)) * 15, 2));
		...
		drawLine(ctx, x + w * 0.5, y + h * 0.3, x + w * 0.65, y + h *
			0.5, color, w * 0.05);
		drawLine(ctx, x + w * 0.5, y + h * 0.3, x + w * 0.35, y + h *
			0.5, color, w * 0.05);
		ctx.strokeStyle = color;
		ctx.lineWidth = h * 0.025;
		ctx.strokeRect(x + w * 0.2, y + h * 0.2, w * 0.6, h * 0.6);
	},
	[ITEM_FUEL]: (ctx, x, y, w, h) => {
